...
| `contest/vs-builds.js` | **两份 build 的陪练在打级赛制下配对对战**(亮主 / 造反 / 关卡都在),`ai-h2h.js` 量不到亮主阶段的东西,这把量得到;也能拿一份 build 打任一参赛提交 |
...
**当前状态**:正式版 **v0.7.19**,测试版 **v0.7.19**。
v0.7.17 是一条**规则修复**,同时落进两版:一墩里「结构对得上」从「组件构成逐字相同」
改成「候选的组件拆得开就盖得住」—— 主 ♥8♥8 现在毙得掉 ♦Q+♦J 的撒牌了
(见 `docs/RULES.md` §E / §J 第 4 条)。
v0.7.16 是一条**规则修复**,同时落进两版:甩牌失败时罚出的「最小的那一组」改成
**先比张数、张数相同再比 `top`**(见 `docs/RULES.md` §D / §J 第 3 条)。
v0.7.15 是一条 AI 改动,同时落进两版:收官抽样里的**底牌按庄家的埋底策略抽**,不再均匀抽
(配对自对弈三批合并 **+1.05 ±0.15 分/局**,见 `docs/CHANGELOG.md`)。
v0.7.14 是一条**规则修复**,同时落进两版:`decompose` 里同序号的对子改成分层,
不再把本来连得上的拖拉机链切短(见 `docs/RULES.md` §S3 ③)。
正式版仍然**不含 v0.7.13**(那是测试版独有的界面改动)。
