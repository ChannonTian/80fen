...
v0.5.0(AI v4)已在 2026-08-08 合入正式版。v0.5.1 / v0.5.2 已在 2026-08-10 一并合入正式版。**此后测试版又领先:测试版 v0.5.6(在上一版基础上再加:持有区间下界、调主候选、副色探路降级),正式版 v0.5.2。**

## 本地运行

双击 `index.html` 即可(需与 `terms-shanghai.js` 同目录)。打开 `index.html#test` 查看引擎自测(191 项)。引擎块导出唯一一份 `ENGINE`,浏览器 `#test` 与 node 无头测试共用同一份,不会两边漂移。
