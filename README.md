...
**当前状态**:正式版 **v0.7.0**,测试版 **v0.7.5**。
测试版领先的部分里,三条问题修复(关卡口径、竖屏发牌遮挡、甩牌诊断)与 AI 强弱无关,
可以随时合入;AI 那一摊对固定参照 v0.7.0 是 **−0.05 ±0.37(打平)**,所以没合。
历史晋级:v0.5.0 于 2026-08-08、v0.5.1~v0.5.2 于 08-10、v0.5.3~v0.7.0 于 08-15 合入正式版。

## 本地运行

双击 `index.html` 即可(需与 `terms-shanghai.js` 同目录)。打开 `index.html#test` 查看引擎自测(正式版 197 项,测试版 209 项)。引擎块导出唯一一份 `ENGINE`,浏览器 `#test` 与 node 无头测试共用同一份,不会两边漂移。
