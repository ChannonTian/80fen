...
**当前状态**:正式版 **v0.7.8**,测试版 **v0.7.9**。
v0.7.9 修的是 `scorePlay` 里一处量纲不同步(收益按「贴之后」记账、概率按「贴之前」算),
需求列表里 8 条报障的共同根因;聚合 +0.25 ±0.56(打平),但它是内部一致性修复,
不是策略取舍 —— 先留测试版实测。
...
双击 `index.html` 即可(需与 `terms-shanghai.js` 同目录)。打开 `index.html#test` 查看引擎自测(正式版 210 项,测试版 219 项)。引擎块导出唯一一份 `ENGINE`,浏览器 `#test` 与 node 无头测试共用同一份,不会两边漂移。
