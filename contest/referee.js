  ...
  for(;;){                                   // 重发就从头再来一局
    ...
    const tryDecl=(seat)=>{
      ...
      if(!isReinforce && !E.canOverride(decl, opt2, seat)){
        vio[seat%2].add('onDeal:压不过当前亮主', JSON.stringify(opt2)); return false;
      }
    ...
    let buried=null;
    {
      ...
      if(!buried) buried=st.fallbackDiscard(hands[declSeat], trump);   // 兜底,局照打
    }
    ...
    return {sc, declSeat, declTeam, trump, redealCount,
...
function playMatch(matchSeed, aiOf, o){
  ...
  while(M.round<opt.maxRounds){
    ...
    rounds.push({no:M.round, declSeat:r.declSeat, declTeam:r.declTeam,
                 total:r.sc.total, defendersWin:r.sc.defendersWin,
