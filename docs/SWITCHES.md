...
### 抽样世界里的底牌按庄家的埋底策略抽(见 sampleWorld)。0 = 均匀(v0.7.14 及以前)。

| 参数 | 默认 | 说明 |
|---|---|---|
| `egKittyModel` | `1` | — |

### 分牌进底牌的相对权重。0.35 与 0.7 同一批种子 +0.78 / +0.99,差在噪声里;

| 参数 | 默认 | 说明 |
|---|---|---|
| `egKittyPtW` | `0.6` | — |

### 抽样世界里把亮主者亮出的那几张(级牌 / 王对)钉在他手里 —— 硬信息,不是推断

| 参数 | 默认 | 说明 |
|---|---|---|
| `egDeclKnown` | `0` | — |
| `egExact` | `0` | 剩这么多张及以下时,抽样世界用完全信息极小极大代替推演(0 = 关) |
...
| `ptLevelBonus` | `25` | 分级(5/10/K)的亮主加成;砍到 0 / 10 打级自对弈 −0.28 / −0.21 级/场,不动 |

### 闲家亮对(见 scoreDeclOption)。对 claude-opus-5 / kimi-k3 各 60 种子打级:级数差

| 参数 | 默认 | 说明 |
|---|---|---|
| `defDeclPair` | `0` | — |
| `defPairBonus` | `10` | 闲家亮对的加成 |
| `defThreshAdd` | `0` | 闲家亮主门槛的增减(负 = 更爱亮) |

### 跨线的即时加成(见 ladderCross)。**默认关 —— 三档全负**:10 → −0.20 ±0.24、
