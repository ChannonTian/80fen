...
| 参数 | 默认 | 说明 |
|---|---|---|
| `egKittyModel` | `1` | 1 = 按埋底策略抽;0 = 均匀(v0.7.14 及以前) |

### 分牌进底牌的相对权重,以及它为什么不算标定死

| 参数 | 默认 | 说明 |
|---|---|---|
| `egKittyPtW` | `0.6` | 分牌进底牌的相对权重(1 = 和闲牌一样容易被埋) |
...
| 参数 | 默认 | 说明 |
|---|---|---|
| `voidReadTrust` | `0.6` | (v0.7.21 测试版默认开)calib-infer:0.8 / 0.6 / 0.4 对数损失 0.353 / 0.354 / 0.355,0.6 分箱最均衡; |
