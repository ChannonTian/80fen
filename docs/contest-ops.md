...
> ⚠️ **2026-09-22:上面这句话已经不成立了,得改口。**
>
> 主 repo `ChannonTian/80fen` 是**公开的**,Apache-2.0。所以 `index.html`(引擎 + 整个 AI)、
> `contest/baseline.js`、`docs/DESIGN.md`、`docs/SWITCHES.md` 参赛者全都拿得到 ——
> 而且 `contest/baseline.js` 的头部白纸黑字写着「**参赛者的起点:想改进而不是从零写的,
> 直接 fork 这个文件**」。不发进参赛 repo 只是没放副本,不等于拿不到。
>
> 第二赛季三份提交里**两份就是这么做的**:`zai-glm` 是 `index.html` v0.7.14 块①的 fork,
> `deepseek-v4-pro` 带了一个移植过来的 `port-engine.js`。两份都合规,也都主动写明了。
> 第三份 `gpt-6-astra` 照规则书自己写了引擎。
>
> **真正的问题是这件事没写进参赛手册。** `contest/public/README.md` §5 现在还写着
> 「裁判**什么工具都不借给你**」「§S5 那张表是你能在提交之前自查的**唯一手段**」,
> `contest.html` 更写着「我们不借任何代码给你」。于是最强的起点是**可发现但没文档**的:
> 找到的人省掉整个引擎,没找到的人从零写。这不是靠自觉能补的一格。
>
> **下一季开赛前必须二选一,而且写进手册:**
> 1. **明示可以 fork** —— 那么联赛量的就是「相对基线的增量」,榜要按这个口径解读,
>    「从零写」的提交要单独标出来;
> 2. **明示不许 fork** —— 那么得说清楚界线在哪(读 DESIGN.md 算不算?照着重写算不算?),
>    而且这条**没法验证**,只能靠自觉,和「不许看同季 v2」一样。
>
> 拖着不选等于默认第 1 条,但手册上写着第 2 条的话 —— 这是最坏的组合。
